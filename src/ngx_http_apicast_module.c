...
int
ngx_http_apicast_ffi_set_proxy_ca_cert(ngx_http_request_t *r, void *cdata_ca)
{
...
int
ngx_http_apicast_ffi_set_ssl_verify(ngx_http_request_t *r, int verify,
    int verify_deph)
{
    ngx_http_apicast_ctx_t *ctx;
    ...
    ngx_http_request_t *r, ngx_http_apicast_ctx_t *ctx, ngx_connection_t *conn)
{
    ...
    rc = SSL_use_certificate(conn->ssl->connection, ctx->proxy_client_cert);
    if (rc == 0) {
    ...
    ngx_connection_t *conn)
{
    ...
    rc = SSL_use_PrivateKey(conn->ssl->connection, ctx->proxy_client_cert_key);
...
ngx_int_t
ngx_http_upstream_should_verified(ngx_http_request_t *r)
{
    ngx_http_apicast_ctx_t *ctx;
...
static ngx_int_t
ngx_http_apicast_init(ngx_conf_t *cf)
{
